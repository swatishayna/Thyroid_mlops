...
def clean_process(config_path):
    ...
    df_allhyper      = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['allhyper']),delimiter=',')
    df_allhypertest  = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['allhypertest']),delimiter=',')
    df_allhypo       = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['allhypo']),delimiter=',')
    df_allhypotest   = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['allhypotest']),delimiter=',')
    df_thyroid0387   = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['thyroid0387']),delimiter=',')
    df_hypothyroid   = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['hypothyroid']),delimiter=',')
    df_sickeuthyroid = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['sickeuthyroid']),delimiter=',')
    df_anntest       = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['anntest']),delimiter=',')
    df_anntrain      = pd.read_csv(os.path.join(raw_dir_path,config['data_source']['anntrain']),delimiter=',')
    ...
    df_allhypertest['Class'] =df_allhypertest[['Target']].apply(removenumber, axis=1)
    df_allhypo['Class']      =df_allhypo[['Target']].apply(removenumber, axis=1)
    df_allhypotest['Class']  =df_allhypotest[['Target']].apply(removenumber, axis=1)


    df_allhyper     = cleanclasscolumn(df_allhyper, ['hyperthyroid', 'T3 toxic', 'goitre','secondary toxic'],'hyperthyroid' )
    df_allhypertest = cleanclasscolumn(df_allhypertest, ['hyperthyroid', 'T3 toxic', 'goitre','secondary toxic'],'hyperthyroid')
